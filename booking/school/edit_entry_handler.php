...
require_once 'teacher_queue.inc.php';
...
// The club website session is the source of identity.  Never trust identity
// fields posted by the booking form.
$school_account = school_require_account($is_ajax);
$mrbs_username = $school_account['email'];
$create_by = $school_account['email'];
$name = $school_account['display_name'];
...
// A completed student booking is final.  Administrators retain MRBS's normal
// edit controls for exceptional school-side corrections.
if (isset($id) && !is_book_admin($rooms))
{
  $message = '预约成功后不能修改或取消。';
  if ($is_ajax)
  {
    invalid_date($message, true);
  }
  invalid_booking($message);
}
...
// The legacy engine uses its own session identity to check room access.
// New bookings keep the independently verified school email as their stored owner.
if (!getWritable(isset($id) ? $create_by : ($mrbs_user->username ?? null), $target_rooms, false))
{
...
// Validate the requested boundaries before MRBS rounding can alter them.
if (!school_trial_slot((int)$start_time,(int)$end_time)) invalid_booking(school_trial_message());
...
// Student bookings are deliberately constrained here as well as in the UI.
// This prevents direct form submissions from bypassing the school's policy.
if (!is_book_admin($rooms))
{
  if (($_POST['school_rules_ack'] ?? '') !== '1')
  {
    invalid_booking('请先阅读预约须知并确认：严禁恶意占场、预约后无故不使用等行为，违者将被禁止预约3楼场地。');
  }

  $booking_timezone = new \DateTimeZone(date_default_timezone_get());
  $next_monday = (new \DateTimeImmutable('monday next week', $booking_timezone))->setTime(0, 0);
  $next_saturday = $next_monday->modify('+5 days');

  if (($start_time < $next_monday->getTimestamp()) || ($end_time > $next_saturday->getTimestamp()))
  {
    $message = '本周只能预约下一周周一至周五的场地。';
    if ($is_ajax)
    {
      invalid_date($message, true);
    }
  ...
  $booking_start_local = (new \DateTimeImmutable('@' . $start_time))->setTimezone($booking_timezone);
  $booking_end_local = (new \DateTimeImmutable('@' . $end_time))->setTimezone($booking_timezone);

  school_booking_lock();
  if ((int)$school_account['level'] !== 2 && !school_student_window()) {
    invalid_booking('学生预约开放时间为周六、周日 13:00–19:00 / Student bookings open Saturday and Sunday, 13:00–19:00.');
  }
  if ((int)$school_account['level'] === 2 && (!$is_ajax || $commit)) {
    try { $result = school_teacher_submit($school_account,(int)$rooms[0],$start_time,$end_time); }
    catch (\Throwable $e) { http_response_code(409);header('Content-Type: application/json');echo json_encode(['error'=>$e->getMessage()]);exit; }
    header('Content-Type: application/json');echo json_encode($result);exit;
  }

  // Bind all limits to the verified website email address.
  if (!isset($id))
  {
    $sql = "SELECT COUNT(*)
              FROM " . _tbl('entry') . "
             WHERE LOWER(TRIM(create_by))=LOWER(TRIM(?))
               AND start_time>=?
               AND start_time<?";
    $weekly_bookings = (int) db()->query1(
      $sql,
      array($school_account['email'], $next_monday->getTimestamp(), $next_saturday->getTimestamp())
    );
    if ($weekly_bookings >= 3)
    {
      $message = '每个账号下一周最多预约三次。';
      if ($is_ajax)
      {
        invalid_date($message, true);
      }
    ...
    $day_start = $booking_start_local->setTime(0, 0);
    $day_end = $day_start->modify('+1 day');
    $daily_bookings = (int) db()->query1(
      $sql,
      array($school_account['email'], $day_start->getTimestamp(), $day_end->getTimestamp())
    );
    if ($daily_bookings > 0)
    {
      $message = '同一账号同一天只能预约一次。';
      if ($is_ajax)
      {
        invalid_date($message, true);
      }
...
foreach ($rooms as $room_id)
{
  ...
  if (!getWritable(isset($id) ? $create_by : ($mrbs_user->username ?? null), $room_id))
  {
    continue;
  }
