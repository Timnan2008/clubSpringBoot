  ...
  return (
    ...
    >
      <button
        ...
        aria-label={`${label}：${options.find((o) => o[0] === value)?.[1] || tr("全部")}`}
      ...
      >
        <span>{options.find((o) => o[0] === value)?.[1] || tr("全部")}</span>
        <svg
          viewBox="0 0 24 24"
          fill="none"
          stroke="currentColor"
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ref={panel}
            className={"category-popup " + (up ? "opens-up" : "")}
            id={id}
            initial={{
              opacity: 0,
              y: -6,
          ...
          >
            ...
            <OptionWheel
              ...
              label={label}
              onChange={(index) => setDraft(options[index][0])}
            />
            <footer>
              <small>{tr("滚动、拖动或方向键选择")}</small>
              <button
                type="button"
                onClick={() => {
                  onChange(draft);
