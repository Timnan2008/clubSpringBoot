...
const personName = (p) => postName(p, en),
  base = "/api/campus-social",
  postUrl = (id) => "/page/wall?post=" + encodeURIComponent(id),
  stamp = (s) => s?.slice(0, 16).replace("T", " ") || "";
const roles = {
  student: tr("学生"),
  teacher: tr("教师"),
  president: tr("社长"),
...
function Icon({ name }) {
  const Component = { heart: Heart, chat: ChatCircle, mail: Envelope }[name] || ChatCircle;
  return <Component size={20} aria-hidden="true" />;
}
...
function ReplyTransition({ children, reduced }) {
  ...
  return (
    <motion.div
      className="reply-composer-transition"
      inert={!present}
...
export default function Post({
  ...
  const [replyTarget, setReplyTarget] = useState(null),
    [replyOpen, setReplyOpen] = useState(detail),
    [deleteReply, setDeleteReply] = useState(null);
  const replyInput = useRef(null),
    replyMounted = useRef(false),
    replyForm = useRef(null),
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        clearTimeout(timer);
        if (entries[0].isIntersecting && !sent && !document.hidden)
          timer = setTimeout(() => {
            if (document.hidden) return;
            sent = true;
            viewWrite
              .current("/posts/views", "POST", { ids: [post.id] })
              .then((d) => {
                if (active) setViews(d.views[post.id]);
              })
              .catch(() => {
                sent = false;
              });
    ...
    const target = postElement.current?.querySelector(".social-post-body>header");
    const observe = () => {
      clearTimeout(timer);
      observer.disconnect();
      if (target && !document.hidden) observer.observe(target);
    };
    observe();
  ...
  useEffect(() => {
    if (!replyMounted.current) {
      replyMounted.current = true;
      return;
    }
    if (!replyOpen) return;
    // 聚焦带来的程序滚动不应被当成用户滚动，否则输入框刚打开就会关闭。
    autoScrollUntil.current = performance.now() + 700;
    const frame = requestAnimationFrame(() => {
      replyInput.current?.focus({ preventScroll: true });
      if (detail && replyTarget)
  ...
  }, [text, replyOpen, detail, replyTarget?.id]);
  ...
  useEffect(() => {
    ...
    const pointer = (e) => {
      const control = e.target.closest(".reply-thread-button,.post-reply-toggle");
      if (!inside(e) && !(control && postElement.current?.contains(control))) close();
    };
    const wheel = (e) => {
      if (e.ctrlKey) return;
      ...
      if (
        ...
        ((e.deltaY > 0 && input.scrollTop + input.clientHeight < input.scrollHeight - 1) ||
          (e.deltaY < 0 && input.scrollTop > 0))
      )
        return;
    ...
    const scroll = (e) => {
      if (e.target === document && performance.now() > autoScrollUntil.current) close();
    };
    const key = (e) => {
      if (
        e.key === "Escape" ||
  ...
  const replyComposer = (
    <form
      ref={(node) => {
        if (node) replyForm.current = node;
      }}
      className="post-reply-composer inline-reply"
      id={"reply-composer-" + post.id + "-" + draftKey}
      onSubmit={(e) => {
        e.preventDefault();
        act(async () => {
    ...
    >
      {replyTarget && (
        <div className="reply-target">
          {tx("回复 ", "Replying to ") + personName(replyTarget.author)}
          <button
            type="button"
            onClick={() => {
              setReplyOpen(false);
      ...
      <div className="inline-reply-row">
        <span className="inline-reply-avatar">
  ...
  return (
    <motion.article
      ...
      id={"post-" + post.id}
      className={
        "social-post" + (post.anonymous ? " is-anonymous" : "") + (!detail ? " post-clickable" : "")
      }
      onClick={
        !detail
          ? (e) => {
              if (
                ...
                location.assign(postUrl(post.id));
            }
          : undefined
      }
    >
      {!post.anonymous && (
        <a
          ...
          href={"/page/user/home?account=" + post.author.id}
      ...
      <div className="social-post-body">
        {(profileFeed ? post.profilePinned : post.wallPinned) && (
        ...
        <header>
          <a
            href={
              !post.anonymous && post.author.id
                ? "/page/user/home?account=" + post.author.id
                : undefined
            }
          >
        ...
        {post.club > 0 && (
          <a className="wall-club-badge" href={"/page/wall?club=" + post.club}>
            {post.clubName} ↗
          </a>
        )}
        ...
        <div className="social-post-actions">
          <span
            ...
            title={tx(
              "同一账号 24 小时内重复浏览不重复计数",
              "Repeat views by the same account within 24 hours count once",
            )}
          >
          ...
          <button
            ...
            disabled={busy}
            aria-expanded={replyOpen && !replyTarget}
            onClick={() => {
              setReplyOpen(replyTarget ? true : !replyOpen);
              setReplyTarget(null);
            }}
          >
            <Icon name="chat" />
            <span>{post.replies || tr("回复")}</span>
          </button>
          {!post.own && !post.anonymous && (
            <a href={"/page/messages?with=" + post.author.id} aria-label={tr("发送私信")}>
              <Icon name="mail" />
            </a>
          )}
        </div>
        ...
        <AnimatePresence initial={false}>
          {replyOpen && profile.canPost && (!detail || !replyTarget) && (
            <ReplyTransition key={draftKey} reduced={reduced}>
              {replyComposer}
            </ReplyTransition>
          )}
        </AnimatePresence>
        {!detail && post.previewReply && (
          <div className="post-featured-comment">
            <a className="post-reply-preview" href={postUrl(post.id)}>
              <Avatar person={post.previewReply.author} />
              <strong>{personName(post.previewReply.author)}</strong>
              <span>{post.previewReply.text}</span>
              {post.previewReply.likes > 0 && <small>♡ {post.previewReply.likes}</small>}
            </a>
            <button
              className="reply-thread-button"
              aria-label={tx("回复这条评论", "Reply to this comment")}
              disabled={busy}
              aria-expanded={replyOpen && replyTarget?.id === post.previewReply.id}
              onClick={() => {
                setReplyTarget(post.previewReply);
                setReplyOpen(true);
              }}
            >
              <Icon name="chat" />
              {post.previewReply.replyCount || ""}
            </button>
          </div>
        )}
        {detail && (
          <section className="social-replies">
            <h2>
              {tx("评论", "Comments")} ({post.replies})
            </h2>
            {!replies ? (
              <p>{tr("正在载入回复…")}</p>
            ) : (
              replies.map((r) => (
                <div className="social-reply" data-reply-id={r.id} key={r.id}>
                  <a
                    className="reply-avatar"
                    href={r.author.id ? "/page/user/home?account=" + r.author.id : undefined}
                    aria-label={
                      r.author.role === "anonymous" ? tr("匿名楼主") : personName(r.author)
                    }
                  >
                    <Avatar person={r.author} />
                  </a>
                  <div className="reply-body">
                    ...
                    <div className="reply-actions">
                      ...
                      <button
                        ...
                        aria-label={tx("回复这条评论", "Reply to this comment")}
                        disabled={busy}
                        aria-expanded={replyOpen && replyTarget?.id === r.id}
                        aria-controls={
                          replyOpen && replyTarget?.id === r.id
                            ? "reply-composer-" + post.id + "-" + r.id
                            : undefined
                        }
                        onClick={() => {
                          setReplyTarget(r);
                      ...
                      {(r.own || profile.admin) && (
                        <button
                          ...
                          aria-label={tx("删除评论", "Delete comment")}
                          disabled={busy}
                          onClick={() => setDeleteReply(r.id)}
                        >
                          <Trash size={17} />
                          {tx("删除", "Delete")}
                        </button>
                    ...
                    <AnimatePresence initial={false}>
                      {replyOpen && profile.canPost && replyTarget?.id === r.id && (
                        <ReplyTransition key={r.id} reduced={reduced}>
                          {replyComposer}
                        </ReplyTransition>
                      )}
                    </AnimatePresence>
            ...
            {replies?.length === 0 && <p>{tr("还没有回复，来聊聊吧。")}</p>}
          </section>
        )}
      </div>
    </motion.article>
