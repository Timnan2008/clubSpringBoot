...
export default function ClubOperations({
  ...
  useEffect(() => {
    if (events.length && !events.some((a) => a.id === activity))
      setActivity((events.filter((a) => a.start <= timestamp()).at(-1) || events[0]).id);
  ...
  const badge = (r, deadline) => (
    <span
      className={
        "ops-status " +
        (r?.status === "submitted" ? "submitted" : deadline < timestamp() ? "late" : "")
      }
    >
        ...
        <>
          <div className="ops-term-bar">
            <label>
              ...
              >
                {!data.terms.length && <option value="">{tr("尚未设置学期")}</option>}
                {data.terms.map((t) => (
                  <option key={t.id} value={t.id}>
                    {localizeTerm(t.name, en)}
                  </option>
                ))}
              </select>
            </label>
            ...
            <button disabled={busy} onClick={() => setNewTerm((v) => !v)}>
          ...
          {current &&
            (editor ? (
              <section className="ws-panel ops-report-editor">
                ...
                <form onSubmit={saveReport}>
                  ...
                  <label>
                    {editor.kind === "proposal" ? tr("社员需求与预期收获") : tr("社员反应与反馈")}
                    <textarea
                      value={form.feedback}
                  ...
                  <p className="ws-help">
                    {tr("在线填写即可提交；如有学校统一表格，可在这里附上。附件不超过 20 MB。")}
                  </p>
                  <div className="ops-actions">
                    ...
                    <button name="intent" value="submitted" className="ws-primary" disabled={busy}>
                      {busy
                        ? tr("保存中…")
                        : form.status === "submitted"
                          ? tr("更新提交")
                ...
                {section === "sessions" && (
                  <section className="ws-panel">
                    <div className="ws-section-head">
                      <div>
                        <h3>{tr("活动签到")}</h3>
                        <p>{tr("未登记的成员不会自动记为缺勤。活动开始后可以登记或更正。")}</p>
                      </div>
                    </div>
                    ...
                    {selected && (
                      <>
                        ...
                        {selected.start > timestamp() && (
                          <p className="ws-help">{tr("这场活动尚未开始，开始后开放签到。")}</p>
                        )}
                        <form
                          onSubmit={(e) => {
                            e.preventDefault();
                        ...
                        >
                              ...
                              <tbody>
                                {[...rows.values()].map((m) => (
                                  <tr key={m.member}>
                                    ...
                                    <td>
                                      <select
                                        aria-label={tr("签到状态：") + m.name + " " + m.member}
                                        disabled={busy || selected.start > timestamp()}
                          ...
                          <div className="ops-actions">
                            ...
                            <button
                              className="ws-primary"
                              disabled={busy || !rows.size || selected.start > timestamp()}
                            >
                              {tr("保存签到")}
                            </button>
                          </div>
