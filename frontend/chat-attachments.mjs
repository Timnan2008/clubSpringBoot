...
const encoder = new TextEncoder(),
  prefix = "chat-files:v1:";
export const MAX_CHAT_FILE = 20 * 1024 * 1024;
export const isChatImage = (type) =>
  ["image/jpeg", "image/png", "image/webp", "image/gif", "image/avif"].includes(type);
const context = (sender, recipient, id) =>
  encoder.encode(["qpwfl-chat-file-v1", sender, recipient, id].join("|"));
export function appendChatFiles(existing, incoming) {
  ...
  if (files.length > 4 || files.reduce((sum, f) => sum + f.size, 0) > 40 * 1024 * 1024)
    throw Error("file-total");
  return files;
}
export async function encryptChatFile(file, sender, recipient) {
  ...
  const encrypted = await crypto.subtle.encrypt(
    { name: "AES-GCM", iv, additionalData: context(sender, recipient, id) },
    key,
...
export async function decryptChatFile(bytes, file, sender, recipient) {
  ...
  const clear = await crypto.subtle.decrypt(
    { name: "AES-GCM", iv: unb64(file.iv), additionalData: context(sender, recipient, file.id) },
    key,
    bytes,
  );
  return new Blob([clear], {
    type: isChatImage(file.type) ? file.type : "application/octet-stream",
  });
}
export async function prepareChatFiles(own, peer, sender, recipient, text, files) {
  if (text.length > 2000 || !files.length) throw Error("Invalid message");
  appendChatFiles([], files);
  const encrypted = [];
  for (const file of files) encrypted.push(await encryptChatFile(file, sender, recipient));
...
export function parseChatFiles(text) {
  ...
  try {
    ...
    for (const f of body.files)
      if (
        !/^[a-f0-9]{8}(?:-[a-f0-9]{4}){3}-[a-f0-9]{12}$/.test(f.id) ||
        typeof f.name !== "string" ||
        f.name.length > 120 ||
        typeof f.type !== "string" ||
        !Number.isInteger(f.size) ||
        f.size < 0 ||
        f.size > MAX_CHAT_FILE ||
        unb64(f.key).length !== 32 ||
        unb64(f.iv).length !== 12
      )
        return null;
    return body;
  } catch {
