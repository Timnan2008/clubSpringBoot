  ...
  if (!response.ok || data.code !== 200)
    throw new Error(
      data.message && data.message !== "错误"
        ...
        : t("操作未成功，请检查信息后重试。", "Please check your details and try again."),
    );
  return data;
}
...
function Auth() {
  ...
  function change(key, value) {
    if (key === "email" || key === "role") setRegistrationVerified(false);
  ...
  function validation(current) {
    ...
    if (current === 3) {
      if (form.role === "user" && !/^[A-Za-z0-9-]{1,32}$/.test(form.studentNumber.trim()))
        return t("请填写学生号。", "Enter your student number.");
      if (!passwordChecks(form.password).every(Boolean))
        return t("请满足下方全部密码要求。", "Meet all password requirements below.");
    ...
    if (current === 4) {
      ...
      if (form.role === "teacher" && !form.email.trim().toLowerCase().endsWith("@shwfl.edu.cn"))
        return t(
          "教师请使用 @shwfl.edu.cn 学校邮箱。",
          "Teachers must use their @shwfl.edu.cn school email.",
        );
      if (!/^\d{6}$/.test(form.code))
        return t("请输入邮件中的 6 位数字验证码。", "Enter the six-digit email code.");
    }
    return "";
  }
  ...
  async function sendCode() {
    ...
    if (form.role === "teacher" && !email.toLowerCase().endsWith("@shwfl.edu.cn")) {
      setError(
        t(
          "教师请使用 @shwfl.edu.cn 学校邮箱。",
          "Teachers must use their @shwfl.edu.cn school email.",
  ...
  const [turnstileToken, setTurnstileToken] = useState(""),
    ...
    [registrationVerified, setRegistrationVerified] = useState(false);
  async function register() {
    if (!turnstileToken && !registrationVerified) {
      setError(t("请先完成人机验证。", "Please complete verification first."));
      return false;
    }
    if (!["user", "teacher"].includes(form.role)) {
      setError(
        t("社长账号由管理员开通。", "Club president accounts are created by an administrator."),
      );
      return false;
    }
    ...
    try {
      ...
      // Redeem the single-use token once. Correcting a code or retrying storage uses
      // the server's five-minute receipt, bound to this browser, email and role.
      try {
        await request("/api/user/registration-verification", {
          email,
          role: form.role,
          turnstileToken,
        });
        ...
        setTurnstileToken("");
        setVerificationReset((v) => v + 1);
        throw failure;
      }
      await request("/api/user/add/" + form.role, {
        ...body,
        turnstileToken,
  ...
  async function signIn(event) {
    ...
    try {
      ...
      try {
        ...
        await syncIdentityAfterLogin(login.password);
        sessionStorage.removeItem("club-message-sync-pending");
      } catch {
        sessionStorage.setItem("club-message-sync-pending", "1");
      }
      ...
      if (next) {
        try {
          ...
          if (
            ...
            !/^\/page\/user\/(login|register)/.test(url.pathname)
          )
      ...
      >
        <section className="auth-intro">
          ...
          <div className="auth-art" aria-hidden="true">
            <Suspense fallback={<div className="color-bends-container" />}>
              <ColorBends
                colors={AUTH_COLORS}
                rotation={90}
                speed={0.2}
        ...
        <section className="auth-card" aria-label={t("账号登录与注册", "Sign in and registration")}>
          <div className="auth-tabs" role="tablist" aria-label={t("账号入口", "Account")}>
            <button
              type="button"
              role="tab"
              id="login-tab"
          ...
          <AnimatePresence mode="wait" initial={false}>
            {mode === "forgot" ? (
              <motion.div
                ...
                exit={{ opacity: 0, x: -24 }}
              >
                <ForgotPassword initialEmail={login.email} onBack={() => switchMode("login")} />
              </motion.div>
            ) : mode === "login" ? (
              <motion.div
                key="login"
              ...
              >
                <header className="auth-heading">
                  ...
                  <span>
                    {t("登录账号，继续你的社团生活。", "Sign in to pick up where you left off.")}
                  </span>
                </header>
                <form onSubmit={signIn} noValidate>
                  ...
                  <label className="auth-remember">
                    <input
                      ...
                      disabled={busy}
                      onChange={(e) => setLogin((v) => ({ ...v, rememberMe: e.target.checked }))}
                    />
                    <span>{t("记住我，30 天免登录", "Remember me for 30 days")}</span>
                  </label>
              ...
              >
                {done ? (
                  ...
                  >
                    ...
                    <button className="auth-primary" onClick={() => switchMode("login")}>
                      {t("去登录 →", "Sign in →")}
                    </button>
                  </motion.div>
                ) : (
                  <>
                    ...
                    <Stepper
                      ...
                      onStepChange={(v) => {
                        ...
                        setTimeout(
                          () =>
                            registerPanel.current?.querySelector(".step-default input")?.focus(),
                          450,
                        );
                      }}
                      ...
                      nextButtonProps={{
                        disabled: busy || sending || (step === 2 && form.role === "club-president"),
                      }}
                      backButtonProps={{ disabled: busy || sending }}
                      backButtonText={t("上一步", "Back")}
                      nextButtonText={t("下一步 →", "Continue →")}
                      completeButtonText={
                        busy ? t("正在创建…", "Creating…") : t("完成注册 →", "Create account →")
                      }
                      renderStepIndicator={({ step: position, currentStep }) => (
                        ...
                        >
                          <span
                            className={
                              "auth-progress-dot " + (currentStep >= position ? "is-active" : "")
                            }
                          >
                    ...
                    >
                      <Step>
                        ...
                        <Field
                          ...
                          label={t("昵称（选填）", "Nickname (optional)")}
                          maxLength={60}
                          value={form.nickname}
                      ...
                      <Step>
                        ...
                        {form.role === "club-president" && (
                          <div className="auth-president-info" role="status">
                            <h4>{t("社长账号由管理员开通", "Created by your administrator")}</h4>
                            <p>
                              {t(
                            ...
                            <button
                              ...
                              onClick={() => switchMode("login")}
                            >
                              {t("已有账号，去登录 →", "Already registered? Sign in →")}
                            </button>
                          </div>
                        )}
                        {form.role === "teacher" && (
                          <div className="auth-clubs">
                            <p>{t("负责的社团（可多选）", "Clubs you supervise (optional)")}</p>
                            {clubLoading ? (
                              <p>{t("加载中…", "Loading…")}</p>
                            ) : clubError ? (
                              <button type="button" onClick={() => setClubRetry((v) => v + 1)}>
                                {t("社团加载失败，点击重试", "Could not load clubs. Retry")}
                              </button>
                            ) : (
                        ...
                        {form.role === "user" && (
                          <>
                            <Field
                              ...
                              label={t("学生号", "Student number")}
                              required
                              maxLength={32}
                              value={form.studentNumber}
                              onChange={(e) => change("studentNumber", e.target.value)}
                            />
                            <p className="auth-help">
                              {t(
                                "一个学生号只能绑定一个账户，绑定后不能修改。",
                                "One account per student number. It cannot be changed after registration.",
                              )}
                            </p>
                          </>
                        )}
                      ...
                      <Step>
                        ...
                        <Field
                          ...
                          placeholder={
                            form.role === "teacher" ? "name@shwfl.edu.cn" : "you@example.com"
                          }
                        />
                        {registrationVerified ? (
                          <p className="auth-hint" role="status">
                            {t(
                              "✓ 安全验证已通过，请完成注册。",
                              "✓ Security check complete. Finish creating your account.",
                            )}
                          </p>
                        ) : (
                          <Turnstile
          ...
          <p className="auth-footnote">
            {t("为青浦世外的每一份热爱而建。", "A place for every passion at QPWFLHS.")}
          </p>
        </section>
      </motion.main>
    </MotionConfig>
