...
export default function ColorBends({
  ...
  useEffect(() => {
    ...
    try {
      ...
      return; // The animated CSS palette remains visible without WebGL.
    }
    ...
    function loop(now = performance.now()) {
      ...
      if (!motionQuery.matches) rafRef.current = requestAnimationFrame(loop);
    }
    function resume() {
      cancelAnimationFrame(rafRef.current);
      lastTime = performance.now();
      if (!failed && !document.hidden && container.clientWidth)
  ...
  useEffect(() => {
    const container = containerRef.current;
    if (!container) return;
    const pointerArea = container.closest(".auth-intro") || container;
