  ...
  return (
      ...
      onSubmit={(e) => {
        ...
        act(async () => {
          ...
          setNotice(
            tx(
              "邮箱已更新，下次请使用新邮箱登录。",
              "Email updated. Use your new email to sign in.",
            ),
          );
          setSent("");
          setPassword("");
    ...
    >
      <div className="account-card-title">
        <h2>{tx("登录邮箱", "Sign-in email")}</h2>
      </div>
      <p className="account-email-current">{profile.email}</p>
      <p className="account-hint">
        {tx(
          "验证新邮箱后即可更换，私信和社团记录会保留。",
          "Verify your new email to change it. Messages and club records will stay with your account.",
        )}
      </p>
      <fieldset disabled={busy} className="email-fields">
        ...
        <div className="account-email-actions">
          <button
            ...
            onClick={() => {
              if (
                ![
                  ...form.current.querySelectorAll('input[type="email"],input[type="password"]'),
                ].every((input) => input.reportValidity())
              )
                return;
              act(async () => {
                const target = email.trim().toLowerCase();
                await write("/me/email/code", "POST", { email: target, currentPassword: password });
                setSent(target);
          ...
          >
            {remaining > 0 ? `${remaining}s` : tx("发送验证码", "Send code")}
          </button>
          {sent && (
            <label>
          ...
          {sent && (
            <button disabled={busy || code.length !== 6} type="submit">
              {tx("确认修改邮箱", "Confirm email change")}
            </button>
          )}
        </div>
      </fieldset>
