...
function Modal({ title, close, busy, children }) {
  const ref = useRef();
  useEffect(() => {
    ...
    const key = (e) => {
      ...
      if (e.key === "Tab") {
        const items = [...panel.querySelectorAll("button,input,textarea,select,a[href]")].filter(
          (n) => !n.disabled,
        );
    ...
    >
      ...
      >
        <header>
          ...
          <button aria-label={tr("关闭弹窗")} disabled={busy} onClick={close}>
            ×
          </button>
        </header>
        {children}
...
function ActivityForm({ application, date, busy, onSave }) {
  return (
    <form
      onSubmit={(e) => {
        e.preventDefault();
    ...
    >
      <label>
        ...
        <input
          ...
          maxLength={100}
          required
          placeholder={application ? tr("例如：校园艺术节展演") : tr("例如：每周社团活动")}
        />
      </label>
      <div className="ws-form-grid">
        <label>
          {tr("开始时间")}
          <input name="start" type="datetime-local" defaultValue={date + "T15:30"} required />
        </label>
        <label>
          {tr("结束时间")}
          <input name="end" type="datetime-local" defaultValue={date + "T16:30"} required />
        </label>
      </div>
      <div className="ws-form-grid">
        <label>
          ...
          <input
            ...
            required
            maxLength={150}
            placeholder={tr("教室、操场或其他场地")}
          />
        </label>
        <label>
          {tr("预计参与人数")}
          <input name="participants" type="number" min="1" max="5000" defaultValue="20" required />
        </label>
      </div>
      <label>
        {application ? tr("活动方案与场地需求") : tr("活动说明")}
        <textarea
          name="description"
          maxLength={3000}
          rows="4"
      ...
      <p className="ws-help">
        {application
          ? tr("提交后由管理员审核，通过后显示在活动日历中。")
          : tr("活动时间按北京时间显示。")}
      </p>
      <button className="ws-primary" disabled={busy}>
        {busy ? tr("正在保存…") : application ? tr("提交申请 →") : tr("保存活动 →")}
      </button>
    </form>
  );
}
function Workspace() {
  ...
  const onDay = (e, d) =>
    e.start.slice(0, 10) <= d &&
    (e.end.slice(0, 10) > d || (e.end.slice(0, 10) === d && !e.end.endsWith("T00:00")));
  ...
  const tabs = [
    ["term", tr("学期材料"), "01"],
    ["sessions", tr("活动签到与反馈"), "02"],
    ["recruitment", tr("招新确认"), "03"],
    ["members", tr("社员管理"), "04"],
    ["calendar", tr("活动日历"), "05"],
    ["profile", tr("社团资料"), "06"],
    ["files", tr("文件资料库"), "07"],
    ["applications", tr("校园活动申请"), "08"],
  ];
  return (
    <MotionConfig reducedMotion="user">
      ...
      <main className="ws-shell">
        <aside className="ws-sidebar">
          <BackButton className="ws-back" fallback="/" label />
          <div className="ws-brand">
            <h1>{tr("社团管理")}</h1>
            <p>{tr("学期材料、活动记录与成员管理")}</p>
          </div>
          {profile && (
            <label className="ws-club-select">
              {tr("当前社团")}
              <select
                aria-label={tr("选择社团")}
                value={club}
          ...
          <div className="ws-sidebar-foot">
            ...
            <div>
              <strong>
                {(en ? profile?.account?.nameEn || profile?.name : profile?.name) || tr("正在载入")}
              </strong>
              <small>{profile?.admin ? tr("管理员工作台") : tr("社团负责人工作台")}</small>
            </div>
          </div>
        </aside>
        <div className="ws-main">
          <header className="ws-page-header">
            <div>
              <p>
                QPWFLHS /{" "}
                {(en ? selected?.nameEn || selected?.name : selected?.name) || tr("我的社团")}
              </p>
          ...
          {!profile ? (
            <Empty>{tr("正在载入工作台…")}</Empty>
          ) : !club ? (
            <Empty>{tr("尚未分配负责的社团，请联系管理员。")}</Empty>
          ) : loading || !data ? (
            <Empty>{loading ? tr("正在载入社团数据…") : tr("暂时无法显示社团数据")}</Empty>
          ) : (
            <>
              <div className="ws-stats">
                ...
                <div>
                  <span>{tx("已安排社团活动", "Scheduled club activities")}</span>
                  <strong>
                    {events.filter((e) => e.kind === "event").length}
                    <small>{tr("场")}</small>
                  </strong>
                </div>
                <div>
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  ...
                  {["term", "sessions", "recruitment"].includes(tab) && (
                    <ClubOperations
                      key={club + tab}
                      club={club}
                      token={profile.token}
                  ...
                  {tab === "calendar" && (
                    <div className="ws-calendar-layout">
                      <section className="ws-panel ws-calendar">
                        <div className="ws-section-head">
                          <div>
                            <h3>
                              <DateJump
                                value={
                                  day.startsWith(iso(month).slice(0, 7))
                                    ? day
                                    : iso(new Date(month.getFullYear(), month.getMonth(), 1))
                                }
                                onChange={(d) => {
                                  setDay(d);
                                  setMonth(new Date(d + "T12:00:00"));
                                }}
                              >
                                {month.getFullYear()}
                                {tr("年")}
                        ...
                        <div className="ws-days">
                          {days.map((date) => {
                            ...
                            return (
                              <button
                                key={d}
                                aria-label={`${d}${list.length ? "，" + list.length + tr(" 场活动") : ""}`}
                                aria-pressed={d === day}
                                className={`${date.getMonth() !== month.getMonth() ? "outside " : ""}${d === day ? "selected " : ""}${d === today() ? "today" : ""}`}
                                onClick={() => setDay(d)}
                              >
                                ...
                                {list.slice(0, 2).map((e) => (
                                  <small
                                    key={e.id}
                                    className={e.kind === "application" ? "campus" : ""}
                                  >
                                    {e.title}
                                  </small>
                                ))}
                                {list.length > 2 && <em>+{list.length - 2}</em>}
                              </button>
                            );
                          })}
                        </div>
                      </section>
                      <aside className="ws-panel ws-agenda">
                        <div className="ws-section-head">
                          <div>
                            <span className="ws-kicker">DAY PLAN</span>
                            <h3>
                              {day.slice(5).replace("-", tr(" 月 "))}
                              {tr("日")}
                            </h3>
                          </div>
                          <button
                        ...
                        {events.filter((e) => onDay(e, day)).length === 0 ? (
                          <Empty>
                            {tr("这一天暂无活动")}
                            <br />
                            {tr("可新增社团活动。")}
                          ...
                          events
                            ...
                            .map((e) => (
                              <article className="ws-agenda-event" key={e.id}>
                                <span>
                                  {e.start.slice(11, 16)} — {e.end.slice(11, 16)}
                                </span>
                                <h4>{e.title}</h4>
                                <small className="ws-badge">
                                  {e.kind === "event"
                                    ? tx("社团活动", "Club activity")
                                    : tx("校园活动", "Campus event")}
                                </small>
                                <p>
                                  {e.location} ·{" "}
                                  {e.participants
                                    ? e.participants + tr("人")
                                    : tx("人数待确认", "Attendance to be confirmed")}
                                </p>
                                <p>{e.description}</p>
                                {e.kind === "event" ? (
                                  <button
                                  ...
                                  <small className="ws-badge approved">
                                    {tr("校园活动 · 已通过")}
                                  </small>
                                )}
                              </article>
                        ...
                        <p className="ws-help">{tr("北京时间 · 点击日期查看活动")}</p>
                      </aside>
                    </div>
                  )}
                  {tab === "members" && (
                  ...
                  {tab === "files" && (
                    <section className="ws-panel">
                      <div className="ws-section-head">
                        <div>
                          <h3>{tr("社团资料库")}</h3>
                          <p>{tr("提交活动方案、总结与社团材料，供指导教师和管理员查看。")}</p>
                        </div>
                        <button
                          className="ws-primary"
                          onClick={() =>
                          ...
                          {data.documents.map((d) => (
                            <article key={d.id}>
                              <span className="ws-file-icon">
                                ...
                                <small>{d.name.split(".").at(-1).toUpperCase().slice(0, 4)}</small>
                              </span>
                              <div>
                                <h4>{d.name}</h4>
                                <p>{d.note || tr("无附加说明")}</p>
                                <small>
                                  {(d.size / 1024).toFixed(1)} KB · {d.createdAt.slice(0, 10)} ·{" "}
                                  {d.submittedBy}
                                </small>
                              </div>
                  ...
                  {tab === "applications" && (
                    <section className="ws-panel">
                      ...
                      {!applications.length ? (
                        ...
                        <div className="ws-application-list">
                          {applications.map((a) => (
                            <article key={a.id}>
                              <div className="ws-section-head">
                                <h4>{a.title}</h4>
                                <span className={"ws-badge " + a.status}>{statuses[a.status]}</span>
                              </div>
                              <p>
                                {a.start.replace("T", " ")} — {a.end.replace("T", " ")} ·{" "}
                                {a.location} · {a.participants}
                                {tr("人")}
                              </p>
                              <p className="ws-description">{a.description}</p>
                              <small>
                                {tr("提交人：")}
                                {a.submittedBy}
                              </small>
                              {a.reviewNote && (
                                <p className="ws-review-note">
                                  {tr("审核意见：")}
                                  {a.reviewNote}
                                </p>
                              )}
                              {profile.admin && a.status === "pending" && (
                                <button
                                  className="ws-primary"
                                  onClick={() =>
                                    open({
        ...
        {modal && (
          ...
          >
            ...
            {["event", "application"].includes(modal.kind) && (
              <ActivityForm
                application={modal.kind === "application"}
                date={modal.kind === "application" && day <= today() ? tomorrow() : day}
                busy={busy}
                onSave={(body) =>
                  perform(modal.kind === "event" ? "/events" : "/applications", "POST", body)
                }
              />
            )}
            {modal.kind === "file" && (
              ...
              >
                <label className="ws-file-drop">
                  <FileArrowUp size={36} aria-hidden="true" />
                  <strong>{uploadName || tr("选择文件")}</strong>
                  <span>{tx("点击选择文件", "Click to choose a file")}</span>
                  <input
                    aria-label={tr("选择文件")}
                ...
                <p className="ws-help">
                  {tr("支持 PDF、Office 文档、文本、图片和 ZIP，单个文件不超过 20 MB。")}
                </p>
                <label>
                  {tr("文件说明")}
                  <textarea
            ...
            {modal.kind === "review" && (
              <form
                onSubmit={(e) => {
                  e.preventDefault();
                  perform(
                    "/applications/" + modal.application.id + "/review",
                    "POST",
                    Object.fromEntries(new FormData(e.currentTarget)),
                  );
                }}
              >
                <p>{modal.application.title}</p>
                <label>
                  {tr("审核结果")}
                  <select name="decision">
                    <option value="approved">{tr("通过申请")}</option>
                    <option value="rejected">{tr("驳回申请")}</option>
                  </select>
                </label>
                <label>
                  {tr("审核意见")}
                  <textarea
                    ...
                    maxLength={1000}
                    rows="3"
                    placeholder={tr("驳回时请说明原因")}
                  />
                </label>
                <button disabled={busy} className="ws-primary">
                  {busy ? tr("正在提交…") : tr("确认审核")}
                </button>
              </form>
            )}
          </Modal>
