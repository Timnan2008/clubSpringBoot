...
export default function AvatarEditor({ profile, onSaved }) {
  ...
  return (
    <section className="account-card avatar-editor">
      <Avatar
        person={{
          ...profile.account,
      ...
      <div className="avatar-editor-info">
        <h2>{tr("头像")}</h2>
        <p className="account-hint">{tr("JPG / PNG，不超过 5 MB。图片将居中裁剪为正方形。")}</p>
          ...
          <label>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                if (
                  next &&
                  (!["image/png", "image/jpeg"].includes(next.type) || next.size > 5 * 1024 * 1024)
                ) {
        ...
        {error && (
          <p className="avatar-status" role="alert">
            {error}
          </p>
        )}
        {notice && (
