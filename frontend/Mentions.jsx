  ...
  for (const m of [...mentions].sort((a, b) => a.start - b.start)) {
    ...
    parts.push(
      <a
        ...
        key={m.start}
        href={"/page/user/home?account=" + encodeURIComponent(m.account)}
      >
...
function caretRect(input, caret) {
  ...
  mark.textContent = input.value.slice(caret) || "\u200b";
  mirror.append(mark);
  document.body.append(mirror);
  const first = mark.getClientRects()[0],
    bounds = mirror.getBoundingClientRect(),
    box = input.getBoundingClientRect();
  const rect = {
    left: box.left + first.left - bounds.left - input.scrollLeft,
    top: box.top + first.top - bounds.top - input.scrollTop,
    height: parseFloat(style.lineHeight) || parseFloat(style.fontSize) * 1.4,
  };
  mirror.remove();
  return rect;
}
...
export default function MentionComposer({
  ...
  function inspect(text, caret, ranges = mentions) {
    setQuery(mentionQuery(text, caret, ranges));
    setActive(0);
  }
  function change(e) {
    const next = e.target.value,
      ranges = editMentions(value, next, mentions);
    ...
    const timer = setTimeout(async () => {
      try {
        const response = await fetch(
          "/api/campus-social/users?keyword=" + encodeURIComponent(query.text.trim()),
  ...
  useEffect(() => {
    const close = (e) => {
      if (!root.current?.contains(e.target)) setQuery(null);
    };
  ...
  useLayoutEffect(() => {
    ...
    const place = () => {
      const field = input.current,
        ...
        caret = caretRect(field, query.end);
      const width = Math.min(320, window.innerWidth - 32),
        gap = 6;
      const viewportBottom = window.visualViewport
        ? window.visualViewport.offsetTop + window.visualViewport.height
        : innerHeight;
      const below = viewportBottom - caret.top - caret.height - gap - 12,
        above = caret.top - 12;
      ...
      setPosition({
        width,
        maxHeight,
        left: Math.max(16, Math.min(caret.left, innerWidth - width - 16)) - bounds.left,
        top: (up ? caret.top - gap - height : caret.top + caret.height + gap) - bounds.top,
      });
    };
    place();
  ...
  function choose(person) {
    ...
    onMentionsChange(next.mentions);
    onChange(next.value);
    setQuery(null);
    setResults([]);
    ...
    <div className="mention-composer" ref={root}>
      <textarea
        ...
        onKeyUp={(e) => {
          if (["ArrowLeft", "ArrowRight", "Home", "End"].includes(e.key))
            inspect(value, e.target.selectionStart);
        }}
        onCompositionStart={() => {
        ...
        aria-activedescendant={query && results[active] ? listId + "-" + active : undefined}
        onKeyDown={(e) => {
          ...
          if (query && e.key === "Escape") {
            ...
            e.stopPropagation();
            setQuery(null);
          } else if (query && results.length && ["ArrowDown", "ArrowUp", "Enter"].includes(e.key)) {
            e.preventDefault();
      ...
      {query && (
        <div
          ...
          id={listId}
          role="listbox"
          aria-label={tx("提及用户", "Mention a user")}
        >
          {results.map((person, i) => (
            ...
            >
              ...
              <span className="mention-person">
                <strong>{postName(person, en)}</strong>
                <small>
                  {person.nickname
                    ? "@" + person.nickname
                    : (en ? person.name : person.nameEn) || tx("校园用户", "Campus member")}
                </small>
              </span>
            </button>
          ))}
          {!results.length && (
            <p role="status">
              {loading
                ? tx("正在查找…", "Searching…")
                : query.text
                  ? tx("没有找到用户", "No matching users")
                  : tx("输入姓名或昵称", "Type a name or nickname")}
            </p>
          )}
        </div>
      )}
