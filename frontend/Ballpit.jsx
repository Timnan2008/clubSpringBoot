/*! Adapted from React Bits Ballpit by David Haz, inspired by Kevin Levron. See REACT-BITS-LICENSE.md. */
...
class x {
  ...
  resize() {
    if (this.isDisposed || this.resizeFrozen) return;
    let e, t;
    if (this.#e.size instanceof Object) {
      e = this.#e.size.width;
  ...
  #b() {
    ...
    if (e !== this.size.pixelRatio) this.renderer.setPixelRatio(e);
    const changed =
      this.canvas.width !== Math.floor(this.size.width * e) ||
      this.canvas.height !== Math.floor(this.size.height * e);
    if (changed) {
  ...
  #w() {
    if (this.#n || this.isDisposed || document.hidden) return;
    const animate = () => {
...
class W {
  ...
  update(e) {
    ...
    if (t.exiting) {
      // Remove only the floor and cursor tether; preserve per-ball momentum
      // and continue sphere contacts throughout the exit.
      if (!this.released) {
        this.released = true;
        this.releaseDelays = releaseBalls(o);
      }
      stepFallingBalls(
        {
          positions: s,
          ...
          count: t.count,
          maxX: t.maxX,
          maxZ: Math.max(t.maxZ, t.maxSize),
          releaseDelays: this.releaseDelays,
        },
        e.delta,
      );
      let below = 0;
      for (let idx = 0; idx < t.count; idx++) {
        if (s[3 * idx + 1] + n[idx] < -t.maxY - 2) below++;
      }
      if (below === t.count && !this.exitFinished) {
...
class Z extends d {
  constructor(e, t = {}) {
    ...
    const environment = generator.fromScene(s, 0.04);
    const n = environment.texture;
    generator.dispose();
    s.dispose();
    const o = new g();
...
function createBallpit(e, t = {}) {
  ...
  i.resize();
  initialize({ ...t, maxX: i.size.wWidth / 2, maxY: i.size.wHeight / 2 });
  const n = new y();
  ...
  const h = S({
    domElement: e,
    onMove() {
      if (!s.config.followCursor || s.config.exiting) return;
  ...
  i.onBeforeRender = (e) => {
    if (c) return;
    accumulator = Math.min(accumulator + e.delta, 0.05);
  ...
  return {
    ...
    updateConfig(newProps) {
      // Keep the outgoing scene in its original camera framing if the task rail opens.
      i.resizeFrozen = !!newProps.exiting;
...
const Ballpit = ({ className = "", followCursor = true, ...props }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // The greeting and composer remain usable without WebGL.
      canvas.hidden = true;
      console.warn("Ballpit unavailable:", error.message);
    }
  ...
  return (
    <canvas
      data-exiting={props.exiting || undefined}
      aria-hidden="true"
      className={className}
      ref={canvasRef}
