...
const scheduleLines = new Map([
  ...
  [
    "调休按当天实际课表安排活动；节假日及临时变动以学校通知为准。 Compensatory school days follow the actual class timetable; holidays and changes follow school notices.",
    "Compensatory school days follow the actual class timetable; holidays and changes follow school notices.",
  ],
  [
    "放学后活动可按年级课表于 16:40 提前开始，18:00 前结束并清理场地。 After-school activities may start at 16:40; finish and clean up by 18:00.",
    "After-school activities may start at 16:40; finish and clean up by 18:00.",
  ],
...
export function localizeCalendar(data, english) {
  return {
    ...
    events: data.events.map((event) =>
      event.kind === "club" ? localizeActivity(event, null, english) : event,
    ),
  };
}
