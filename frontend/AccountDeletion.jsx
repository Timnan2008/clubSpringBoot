...
export default function AccountDeletion({ write, account }) {
  ...
  return (
    <section className="account-card account-deletion">
      ...
      <p>
        {tx(
          "永久删除个人资料、装扮、帖子、评论、聊天、日历和所有场地预约，同时退出社团并移除职务。此操作无法恢复。",
          "Permanently delete your profile, appearance, posts, comments, chats, calendar and all court bookings, and remove club memberships and roles. This cannot be undone.",
        )}
      </p>
      {!open ? (
        <button className="account-danger" onClick={() => setOpen(true)}>
        ...
        <form onSubmit={(e) => e.preventDefault()}>
          ...
          <label>
            {tx("输入 DELETE 确认永久注销", "Type DELETE to confirm permanent deletion")}
            <input
              value={confirmation}
              autoComplete="off"
