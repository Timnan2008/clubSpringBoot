...
export function postName(person, english = false) {
  return (
    clean(person?.nickname) || clean(english ? person?.nameEn : person?.name) || realNames(person)
  );
}
...
export function searchName(person, query) {
  ...
  const candidates = ["name", "nameEn", "nickname"]
    .map((field) => {
      const text = clean(person?.[field]),
        value = text.toLocaleLowerCase();
      return {
        field,
        text,
        score: value === q ? 3 : value.startsWith(q) ? 2 : value.includes(q) ? 1 : 0,
      };
    })
