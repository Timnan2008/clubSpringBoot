...
test("second and third mentions preserve earlier account ranges", () => {
  ...
  assert.deepEqual(
    state.mentions.map((m) => m.account),
    ["tim", "bob", "nan"],
  );
  assert.deepEqual(
    state.mentions.map((m) => state.value.slice(m.start, m.end)),
    ["@Tim Tian", "@Bob", "@南天"],
  );
});
test("edits before mentions shift offsets; partial deletion removes only affected mention", () => {
  const ranges = [
    { start: 0, end: 4, account: "a" },
    { start: 5, end: 9, account: "b" },
  ];
  assert.deepEqual(
    editMentions("@Ann @Bob", "Hi @Ann @Bob", ranges),
    ranges.map((m) => ({ ...m, start: m.start + 3, end: m.end + 3 })),
  );
  assert.deepEqual(editMentions("@Ann @Bob", "@An @Bob", ranges), [
    { start: 4, end: 8, account: "b" },
  ]);
...
test("trackpad horizontal momentum stays native, vertical wheel follows distance and direction", () => {
  ...
  const dispose = installClubScroll(container);
  let prevented = 0;
  const wheel = (dy) =>
    handlers.wheel({ deltaX: 0, deltaY: dy, deltaMode: 0, preventDefault: () => prevented++ });
  wheel(-20);
