...
export default function HoldButton({
  children = tx("长按删除", "Hold to delete"),
  doneLabel = tx("已删除", "Deleted"),
  icon = null,
  doneIcon = null,
  ...
  const latest = useRef({ onHold, disabled });
  latest.current = { onHold, disabled };
  ...
  const complete = async () => {
    ...
    if (performance.now() - gesture.current.start < holdTime - 20) return;
    clearTimers();
    const kind = inputRef.current;
    gesture.current.pointerId = null;
    go("pending", kind);
    try {
      const result = await latest.current.onHold?.();
      ...
      if (!alive.current) return;
      setError(error.message || tx("操作失败，请重试", "Please try again"));
      go("idle");
      drive(0, releaseTime, EASE_OUT);
    }
  };
  ...
  useEffect(() => {
    if (disabled) releaseRef.current({ drifted: true });
  }, [disabled]);
  ...
  return (
    <span className="hold-button-wrap">
      <button
        ref={buttonRef}
        type="button"
      ...
      >
        ...
        <span id={hintId} className="hold-button__sr">
          {tx(
            `按住 ${Math.round(holdTime / 100) / 10} 秒确认，提前松开取消`,
            `Hold for ${Math.round(holdTime / 100) / 10} seconds to confirm; release early to cancel`,
          )}
        </span>
      </button>
      {error && (
        <span className="hold-button__error" role="alert">
          {error}
        </span>
      )}
    </span>
