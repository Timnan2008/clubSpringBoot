...
public class WorkspacePageController {
    ...
    public WorkspacePageController(WorkspaceAccess access) {
        this.access = access;
    }
    ...
    public String page(HttpServletRequest request, Model model) {
        try {
            model.addAttribute("loginUser", access.current(request));
            return "page/club-workspace";
        } catch (ResponseStatusException e) {
            return e.getStatusCode().value() == 401
                ? "redirect:/page/user/login?next=%2Fpage%2Fclub%2Fworkspace"
                : "redirect:/page/my-clubs";
        }
    }
}
