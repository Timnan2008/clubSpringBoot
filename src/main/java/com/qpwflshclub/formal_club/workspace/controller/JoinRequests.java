...
public class JoinRequests {
    ...
    public JoinRequests(@Value("${club.join-dir:./data/join-requests}") String path) {
        root = Path.of(path);
    }

    public synchronized List<Entry> read(int club) throws IOException {
        Path p = root.resolve(club + ".json");
        return Files.exists(p)
            ? json.readValue(Files.readAllBytes(p), new TypeReference<ArrayList<Entry>>() {})
            : new ArrayList<>();
    }
    ...
    public synchronized Entry apply(
        int club,
        String account,
        String name,
        String nameEn,
        ...
        var pending = entries
            .stream()
            .filter(e -> e.account().equals(account) && e.status().equals("pending"))
            .findFirst();
    ...
    public synchronized Entry decide(int club, String id, String status, String reviewer)
        throws IOException {
        var entries = read(club);
        var e = entries
            .stream()
        ...
        if (!Set.of("approved", "declined").contains(status)) throw WorkspaceStore.bad(
            "处理结果无效"
        );
        if (e.status().equals(status)) return e;
        if (!e.status().equals("pending")) throw WorkspaceStore.bad("申请已处理");
        var next = new Entry(
            e.id(),
            e.account(),
            e.name(),
