...
public class WorkspaceController {
    ...
    public Map<String, Object> bootstrap(HttpServletRequest request) {
        ...
        return Map.of(
            "name",
            Objects.toString(user.getUsername(), "社团负责人"),
            "admin",
            access.admin(user),
            "account",
            accounts == null
            ...
            access
                ...
                .map(c ->
                    Map.of(
                        ...
                        c.getId(),
                        "name",
                        Objects.toString(c.getClubName(), "社团"),
                        "nameEn",
    ...
    @GetMapping("/{club}")
    public WorkspaceStore.Data data(@PathVariable int club, HttpServletRequest request)
        throws IOException {
        require(club, request, false);
        var data = store.read(club);
        return new WorkspaceStore.Data(
            data
                .documents()
                .stream()
                .map(d ->
                    new WorkspaceStore.Document(
                        d.id(),
                        d.name(),
                        d.size(),
                        d.note(),
                        displayAuthor(d.submittedBy()),
                        d.createdAt()
                    )
                )
                .toList(),
            data
                ...
                .map(a ->
                    new WorkspaceStore.Activity(
                        ...
                        a.kind(),
                        a.status(),
                        displayAuthor(a.submittedBy()),
                        a.createdAt(),
                        a.reviewNote(),
                        displayAuthor(a.reviewedBy())
                    )
                )
                .toList()
        );
    ...
    private String displayAuthor(String email) {
        if (email == null || email.isBlank() || accounts == null) return Objects.toString(
            email,
            ""
        );
        var person = accounts
            .directory()
            .get(SchoolAccounts.key(email));
    ...
    public List<Map<String, Object>> members(@PathVariable int club, HttpServletRequest request) {
        ...
        students.findAll().forEach(s -> {
            if (contains(s.getClubs(), club)) result.put(
                s.getEmail().toLowerCase(Locale.ROOT),
                member(s, "student", "member")
            );
        });
        presidents.findAll().forEach(p -> {
            boolean main = p.getMainClub() != null && Objects.equals(p.getMainClub().getId(), club);
            if (main || contains(p.getClubs(), club)) result.put(
                p.getEmail().toLowerCase(Locale.ROOT),
                member(
                    p,
                    "president",
                    main ? (p.isVicePresident() ? "vice_president" : "president") : "member"
                )
            );
        });
        if (admins != null) admins.findAll().forEach(a -> {
            if (contains(a.getClubs(), club)) result.put(
                a.getEmail().toLowerCase(Locale.ROOT),
                member(a, "admin", "member")
            );
        });
        if (officers != null) for (var o : officers.all())
            if (o.club() == club) {
                var u = accounts.find(o.account());
                result.put(
                    u.getEmail().toLowerCase(Locale.ROOT),
                    member(u, "president", o.position())
                );
            }
    ...
    private Map<String, Object> member(UserBase u, String type, String role) {
        return Map.of(
            ...
            u.getId(),
            "name",
            Objects.toString(u.getUsername(), ""),
            "nameEn",
            Objects.toString(u.getUsernameEn(), ""),
            "type",
            type,
            "role",
            role,
            "avatarUrl",
            accounts == null ? "" : accounts.view(u).avatarUrl(),
            "nickname",
            accounts == null ? "" : accounts.view(u).nickname(),
            "grade",
    ...
    private boolean contains(List<Club> clubs, int id) {
        return clubs != null && clubs.stream().anyMatch(c -> Objects.equals(c.getId(), id));
    }
    ...
    public List<Map<String, Object>> search(
        ...
        students.findAll().forEach(s -> {
            if (
                results.size() < 20 &&
                !contains(s.getClubs(), club) &&
                (Objects.toString(s.getUsername(), "").toLowerCase(Locale.ROOT).contains(q) ||
                    Objects.toString(s.getUsernameEn(), "").toLowerCase(Locale.ROOT).contains(q) ||
                    (accounts != null &&
                        accounts.view(s).nickname().toLowerCase(Locale.ROOT).contains(q)))
            ) results.add(member(s, "student", "member"));
        });
        return results;
    }
    ...
    public Map<String, String> addMember(
        ...
        List<Club> next = new ArrayList<>(
            target.getClubs() == null ? List.of() : target.getClubs()
        );
        if (!contains(next, club)) next.add(access.require(actor, club));
        target.setClubs(next);
        students.save(target);
    ...
    public Map<String, String> removeMember(
        ...
        if (type.equals("student")) {
            ...
            next.removeIf(c -> Objects.equals(c.getId(), club));
            target.setClubs(next);
            students.save(target);
            ...
            ClubPresident target = presidents
                .findById(id)
                .orElseThrow(() -> WorkspaceStore.bad("社员不存在"));
            if (
                target.getMainClub() != null && Objects.equals(target.getMainClub().getId(), club)
            ) throw WorkspaceStore.bad("社长与副社长的任免由管理员处理");
            List<Club> next = new ArrayList<>(
                target.getClubs() == null ? List.of() : target.getClubs()
            );
            next.removeIf(c -> Objects.equals(c.getId(), club));
            target.setClubs(next);
            presidents.save(target);
        } else throw WorkspaceStore.bad("社员类型无效");
        return Map.of("message", "已移出社团");
    }

    @PostMapping("/{club}/documents")
    public WorkspaceStore.Document upload(
        @PathVariable int club,
        ...
        return store.upload(club, file, note, user.getEmail());
    }
        ...
        return ResponseEntity.ok()
            ...
            .header(
                ...
                ContentDisposition.attachment()
                    .filename(document.name(), StandardCharsets.UTF_8)
                    .build()
                    .toString()
            )
    ...
    public WorkspaceStore.Activity create(
        @PathVariable int club,
        @PathVariable String kind,
        @RequestBody ActivityInput input,
        HttpServletRequest request
    ) throws IOException {
        UserBase user = require(club, request, true);
        String title = text(input.title(), 100, "活动名称"),
            location = text(input.location(), 150, "活动地点"),
            description = text(input.description(), 3000, "活动说明");
        LocalDateTime start, end;
        try {
            start = LocalDateTime.parse(input.start());
            end = LocalDateTime.parse(input.end());
        } catch (RuntimeException e) {
            throw WorkspaceStore.bad("请选择有效的开始和结束时间");
        }
        if (!end.isAfter(start) || end.isAfter(start.plusDays(14))) throw WorkspaceStore.bad(
            "结束时间应晚于开始时间，活动不能超过 14 天"
        );
        if (
            input.participants() < (kind.equals("events") ? 0 : 1) || input.participants() > 5000
        ) throw WorkspaceStore.bad("参与人数须为 1–5000 人");
        boolean application = kind.equals("applications");
        if (
            application && !start.isAfter(LocalDateTime.now(java.time.ZoneId.of("Asia/Shanghai")))
        ) throw WorkspaceStore.bad("请申请未来的活动时间");
        return store.add(
            club,
            new WorkspaceStore.Activity(
    ...
    public Map<String, String> removeEvent(
        @PathVariable int club,
        @PathVariable String id,
        HttpServletRequest request
    ) throws IOException {
        require(club, request, true);
        if (operations != null) {
            var records = operations.read(club);
            if (
                records
                    .reports()
                    .stream()
                    .anyMatch(r -> r.activity().equals(id)) ||
                records
                    .attendance()
                    .stream()
                    .anyMatch(a -> a.activity().equals(id))
            ) throw WorkspaceStore.bad("该活动已有签到或反馈记录，不能删除");
        }
        store.removeEvent(club, id);
        return Map.of("message", "活动已删除");
    }

    public record ReviewInput(String decision, String note) {}

    @PostMapping("/{club}/applications/{id}/review")
    public WorkspaceStore.Activity review(
        @PathVariable int club,
        @PathVariable String id,
        @RequestBody ReviewInput input,
        HttpServletRequest request
    ) throws IOException {
        UserBase actor = access.reviewer(request);
        access.mutation(request);
        if (!access.admin(actor)) throw new ResponseStatusException(
    ...
    public ResponseEntity<?> error(ResponseStatusException error) {
        return ResponseEntity.status(error.getStatusCode()).body(
            Map.of("message", Objects.toString(error.getReason(), "请求未成功"))
        );
    }

    @ExceptionHandler(IOException.class)
    public ResponseEntity<?> storageError(IOException error) {
