...
public class TeacherDayGifts {
    ...
    public TeacherDayGifts(
        @Value("${club.teacher-day-file:./data/accounts/teacher-day-2026.json}") String path,
        TeacherRepository teachers
    ) throws IOException {
        this.file = Path.of(path).toAbsolutePath();
        this.teachers = teachers;
        recipients = Files.exists(file)
            ? json.readValue(
    ...
    public static boolean celebration(LocalDate date) {
    ...
    public synchronized void distribute() throws IOException {
        ...
        if (!before.equals(recipients)) try {
            persist();
        } catch (IOException e) {
            recipients.clear();
            recipients.addAll(before);
            throw e;
        }
    }
    ...
    public synchronized boolean awarded(UserBase user) {
        ...
        if (!recipients.contains(id) && today()) {
            ...
            try {
                ...
                throw new IllegalStateException("Unable to save teacher gift", e);
            }
        }
    ...
    private void persist() throws IOException {
        ...
        Path tmp = Files.createTempFile(file.getParent(), "teacher-gifts-", ".tmp");
        try {
            Files.write(tmp, json.writeValueAsBytes(recipients));
            Files.move(
                tmp,
                file,
    ...
    public synchronized void removeAccount(String id) throws IOException {
        if (recipients.remove(id)) persist();
    }
}
