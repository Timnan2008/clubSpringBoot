...
public final class ContentModeration {
    ...
    public static void check(String text) {
        String normalized = normalize(text),
            compact = normalized.replaceAll("[\\s\\p{P}]+", "");
        String words =
            "傻逼,操你妈,草你妈,去死,杀你全家,废物东西,fuck,shit,bitch," +
            Objects.toString(System.getenv("CLUB_BLOCKED_WORDS"), "");
        for (String word : words.split(",")) {
            word = normalize(word.trim());
            if (word.isBlank()) continue;
            boolean match = word.matches("[a-z]+")
                ? Pattern.compile("(?<![a-z])" + Pattern.quote(word) + "(?![a-z])")
                      .matcher(normalized)
                      .find()
                : compact.contains(word);
            if (match) throw SchoolAccounts.error(
                400,
