...
public class ModerationPenalty {
    ...
    public synchronized Outcome strike(String account, String where, String word) {
        ...
        long banUntil = days > 0
            ? Math.max(old.banUntil(), now + days * 86_400_000L)
            : old.banUntil();
    ...
    public synchronized long remainingMillis(String account) {
        return account == null ? 0 : Math.max(0, state(account).banUntil() - System.currentTimeMillis());
    }
    ...
    private void persist() {
        try {
            ...
            Path tmp = Files.createTempFile(file.toAbsolutePath().getParent(), "penalties-", ".tmp");
            try {
