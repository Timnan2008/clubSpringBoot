...
public class RememberMeFilter implements Filter {
    ...
    public RememberMeFilter(RememberMeService remember) {
    ...
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain)
        ...
        if (
            old != null &&
            old.getAttribute("authenticatedEmail") instanceof String email &&
            revocations != null &&
            revocations.revoked(email, old.getCreationTime())
        ) old.invalidate();
        if (
            !request.getRequestURI().startsWith("/javascript/") &&
            !request.getRequestURI().startsWith("/css/") &&
            !request.getRequestURI().startsWith("/other%20photo/")
        ) remember.restore(request, response);
        boolean media =
            (request.getMethod().equals("GET") || request.getMethod().equals("HEAD")) &&
            request
                .getRequestURI()
                .matches(
                    "/api/campus-social/(avatars/[a-f0-9]{64}|appearance/files/[a-f0-9-]{36}(-small)?\\.(png|jpg|webp)|posts/[a-f0-9-]{36}/files/[a-f0-9-]{36})"
                );
        if (
            !media &&
