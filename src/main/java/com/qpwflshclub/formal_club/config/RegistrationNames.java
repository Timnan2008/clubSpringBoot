...
public final class RegistrationNames {
    ...
    public static void validate(String chinese, String english) {
        if (
            (chinese == null || chinese.isBlank()) && (english == null || english.isBlank())
        ) throw SchoolAccounts.error(400, "中文名和英文名至少填写一项 / Enter at least one name");
        for (String value : new String[] { chinese, english })
            if (
                value != null &&
