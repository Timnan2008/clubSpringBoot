...
public class RememberMeService {
    ...
    public RememberMeService(
        @Value("${club.remember-dir:./data/remember}") String directory,
        IUserService users
    ) {
    ...
    private String token(HttpServletRequest request) {
        if (request.getCookies() != null) for (Cookie c : request.getCookies())
            if (
                COOKIE.equals(c.getName()) && c.getValue().matches("[a-f0-9]{64}")
            ) return c.getValue();
        return null;
    }
    ...
    public synchronized void revoke(HttpServletRequest request, HttpServletResponse response) {
        String token = token(request);
        if (token != null) try {
            Files.deleteIfExists(root.resolve(hash(token) + ".json"));
        } catch (java.io.IOException e) {
            throw new IllegalStateException("Cannot revoke remembered login", e);
        }
    ...
    public synchronized void issue(
        ...
        try {
            ...
            Files.write(
                ...
                json.writeValueAsBytes(
                    new Ticket(
                        ...
                        Instant.now().getEpochSecond() + DAYS30
                    )
                ),
            ...
            throw new IllegalStateException("Cannot remember login", e);
        }
    }

    public synchronized void restore(HttpServletRequest request, HttpServletResponse response) {
        if (
            request.getSession(false) != null &&
            request.getSession(false).getAttribute("authenticatedEmail") != null
        ) return;
        String token = token(request);
        ...
        try {
            ...
            Ticket ticket = json.readValue(Files.readAllBytes(file), Ticket.class);
            UserBase user =
                ticket.expires() > Instant.now().getEpochSecond()
                    ? users.findByEmail(ticket.email())
                    : null;
            if (
                user == null ||
                !MessageDigest.isEqual(
                    ticket.passwordVersion().getBytes(StandardCharsets.UTF_8),
                    hash(user.getPassword()).getBytes(StandardCharsets.UTF_8)
                )
            ) {
                revoke(request, response);
                return;
