...
public class TurnstileService {
    ...
    public void verify(String token, String action) {
        ...
        if (token == null || token.isBlank() || token.length() > 2048) throw SchoolAccounts.error(
            400,
            "请完成人机验证"
        );
        JsonNode result;
        try {
            ...
            throw SchoolAccounts.error(503, "验证服务暂不可用，请重试");
    ...
    void validate(JsonNode result, String action) {
        if (
            ...
            !hostnames.contains(result.path("hostname").asText().toLowerCase(Locale.ROOT)) ||
            !action.equals(result.path("action").asText())
