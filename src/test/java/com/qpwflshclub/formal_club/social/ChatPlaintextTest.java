...
class ChatPlaintextTest {
    ...
    void setup() throws Exception {
        ...
        when(recovery.get(SENDER)).thenReturn(new MessageRecovery.Identity(senderPub, senderPriv));
    ...
    void idsAreBoundIntoTheKeyDerivation() throws Exception {
        ...
        byte[] out = cipher.doFinal(TEXT.getBytes(StandardCharsets.UTF_8));
        assertThat(
            chat.decrypt(SENDER, RECIPIENT, wrap(out, iv, senderFingerprint, recipientFingerprint))
        ).isNull();
    }
    ...
    void malformedPayloadsReturnNullInsteadOfThrowing() throws Exception {
        ...
        assertThat(
            chat.decrypt(
                SENDER,
                RECIPIENT,
                "e2ee:v1:" + json.writeValueAsString(json.createObjectNode().put("v", 2))
            )
        ).isNull();
    }
    ...
    void wrongIvLengthOrShortCiphertextIsRejected() throws Exception {
        ...
        assertThat(
            chat.decrypt(
                SENDER,
                RECIPIENT,
                wrap(new byte[40], sixteen, senderFingerprint, recipientFingerprint)
            )
        ).isNull();
        assertThat(
            chat.decrypt(
                SENDER,
                RECIPIENT,
                wrap(new byte[8], iv(), senderFingerprint, recipientFingerprint)
            )
        ).isNull();
    }
