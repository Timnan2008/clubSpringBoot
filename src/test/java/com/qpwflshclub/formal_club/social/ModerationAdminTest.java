...
class ModerationAdminTest {
    ...
    private void asTeacher() {
        when(accounts.current(request)).thenReturn(new com.qpwflshclub.formal_club.User.pojo.Teacher());
    }
    ...
    void teachersCanReadButNotWriteAndAdminsCanDoBoth() {
        ...
        assertThatThrownBy(() -> controller.addWord(
            new ModerationAdminController.WordInput("测试词"), request))
            .hasMessageContaining("403");

        asAdmin();
        assertThat(controller.words(request)).isNotNull();
        assertThat(controller.addWord(new ModerationAdminController.WordInput("测试词"), request))
            .isNotNull();
    }
    ...
    void addedWordTakesEffectImmediatelyAndCanBeRemoved() {
        ...
        assertThat(ContentModeration.wordInfo())
            .anyMatch(info -> info.word().equals("测试违禁词") && info.source().equals("本机"));

        controller.removeWord("测试违禁词", request);
        assertThat(ContentModeration.find("这是测试违禁词")).isNull();
        assertThat(ContentModeration.wordInfo())
            .noneMatch(info -> info.word().equals("测试违禁词") && info.source().equals("本机"));
    }
    ...
    void packagedWordsAreReadOnlyAndDuplicatesAreRejected() {
        asAdmin();
        // 打包词库里的词不能重复加、也不能删
        assertThatThrownBy(() -> controller.addWord(new ModerationAdminController.WordInput("傻逼"), request))
            .hasMessageContaining("打包词库");
        assertThatThrownBy(() -> controller.removeWord("傻逼", request)).hasMessageContaining("只读");

        controller.addWord(new ModerationAdminController.WordInput("新增词条"), request);
        assertThatThrownBy(() -> controller.addWord(new ModerationAdminController.WordInput("新增词条"), request))
    ...
    void invalidWordsAreRejected() {
        ...
        for (String bad : new String[]{"", "   ", "带 空格", "带\n换行"}) {
            assertThatThrownBy(() -> controller.addWord(new ModerationAdminController.WordInput(bad), request))
    ...
    void strikesAreListedAndCanBeForgiven() {
        ...
        Map<?, ?> forgiven = (Map<?, ?>) controller.forgive(
            new ModerationAdminController.ForgiveInput(account), request);
    ...
    void forgiveWithoutAnAccountIsRejected() {
        asAdmin();
        assertThatThrownBy(() -> controller.forgive(
            new ModerationAdminController.ForgiveInput("  "), request)).hasMessageContaining("缺少账号");
    }
}
